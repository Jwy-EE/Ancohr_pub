...
class GaussianRBFInterpolator1D:
    """
    Continuous Differentiable 1D Gaussian Radial Basis Interpolation Operator.
    (Proposed in ANCHOR Architecture to counteract discrete RFFT quantization errors)
    """
    @staticmethod
    def interpolate_fast(x, positions, sigma=1.0, window_size=7):
        ...
        # Normalize coordinates back to continuous physical indices
        original_indices = (positions_flat + 1.0) / 2.0 * (L - 1.0)

        # 1. Establish Physical Anchors
        ...
        sample_indices_int = center_indices.unsqueeze(-1) + window_offsets.view(1, 1, -1)
        sample_indices_int = torch.clamp(sample_indices_int, 0, L - 1)

        # 2. Cross-domain Feature Extraction
        sample_indices_expanded = sample_indices_int.unsqueeze(1).expand(-1, C, -1, -1)
        x_expanded = x.unsqueeze(-2).expand(-1, -1, num_positions, -1)
        sampled_values = torch.gather(x_expanded, dim=-1, index=sample_indices_expanded)

        # 3. Continuous Distance & Gaussian Gravitational Pull (Infinite Differentiability)
        distances = sample_indices_int.float() - original_indices.unsqueeze(-1) 
        weights = torch.exp(-distances**2 / (2 * sigma**2))
        weights = weights / (weights.sum(dim=-1, keepdim=True) + 1e-8)

        # 4. Weighted Summation
        weights = weights.unsqueeze(1) 
        sampled = torch.sum(weights * sampled_values, dim=-1)  

        return sampled.unsqueeze(2) 
...
class DCNv4_1D(nn.Module):
    """
    Deformable Operator (DefOp) instance in ANCHOR. 
    Maintains compatibility with DCNv4_1D import nomenclature.
    """
    def __init__(
        self,
        channels=64,
    ...
    def forward(self, x):
        ...
        # ========== Continuous Phase Compensation (Gaussian RBF) ==========
